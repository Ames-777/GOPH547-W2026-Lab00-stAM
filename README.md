...
To download or clone the repository, click on the green "< > Code" button above the repository files. You can either download entire repository as a ZIP file or single files if selected, or you can clone them using HTTPS, SSH, or GitHub CLI through Python, making sure to navigate to the directory you'd like to store the files to beforehand, using the *cd /path/to/directory/* function.
